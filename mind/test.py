...
def calculate_hit_at_k(sorted_parent_scores, true_parents, k):
    """
    计算 Hit@k 指标。
    :param sorted_parent_scores: 按得分降序排列的预测父节点列表。
    :param true_parents: 测试节点的真实父节点集合。
    :param k: 评估的 top-k 范围。
    :return: 如果 top-k 中有任何一个预测父节点在真实父节点集合中，则返回 1，否则返回 0。
    """
...
def calculate_recall_at_k(sorted_parent_scores, true_parents, k):
    ...
    :param true_parents: 测试节点的真实父节点集合。
    :param k: 评估的 top-k 范围。
    :return: Recall@k 值，表示 top-k 中正确预测的真实父节点的比例。
    """
    top_k_predictions = [score["parent"] for score in sorted_parent_scores[:k]]
...
# input_text = (
#     "You are a knowledgeable language model capable of understanding hierarchical relationships between concepts. Your task is to determine if a candidate concept is a hypernym (a broader or higher-level concept) of a given query concept. A hypernym is defined as a concept that is more abstract, general, or overarching and can encompass the full meaning of the query concept.\nDetermine whether the following is true and answer 'Yes' or 'No'."
#     + 'Query: "{}", Candidate: "{}" Is the candidate a hypernym of the query? <label_token>.'
# )
...
#         LABEL_TOKEN_IDS = tokenizer.convert_tokens_to_ids(LABEL_TOKEN)
#         labels_pos = (inputs["input_ids"] == LABEL_TOKEN_IDS).nonzero(as_tuple=False)

#         for pos, parent_node in zip(labels_pos, batch):
#             batch_idx, mask_idx = pos[0].item(), pos[1].item()
#             label_logits = logits[batch_idx, mask_idx, :]

#             yes_score = label_logits[yes_id].item()
#             no_score = label_logits[no_id].item()
...
#         print(f"Rank {rank}: Parent: {parent_score['parent']}, Score: {parent_score['score']:.4f}")

# output_file = "test_result_taco.json"
